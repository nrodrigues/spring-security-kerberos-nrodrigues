...
import org.springframework.security.extensions.kerberos.ntlm.NtlmServiceRequestToken;
import org.springframework.security.extensions.kerberos.ntlm.NtlmType2MessageException;
import org.springframework.security.extensions.kerberos.spnego.DecodingException;
import org.springframework.security.extensions.kerberos.spnego.SpnegoConstants;
import org.springframework.security.extensions.kerberos.spnego.SpnegoInitToken;
import org.springframework.security.extensions.kerberos.spnego.SpnegoTargToken;
import org.springframework.security.extensions.kerberos.spnego.SpnegoToken;
...
public class SpnegoAuthenticationProcessingFilter extends GenericFilterBean {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		if ((header != null) && header.startsWith("Negotiate ")) {
			...
			SpnegoToken token;
			try {
				token = SpnegoToken.parse(decodedToken);

				if (logger.isDebugEnabled()) {
					logger.debug("Decoded Negotiate header: " + token);
				}
			...
			if (StringUtils.hasText(token.getMechanism())
					&& (token.getMechanism().equals(SpnegoConstants.KERBEROS_MECHANISM) || token.getMechanism().equals(
							SpnegoConstants.LEGACY_KERBEROS_MECHANISM))) {
				if (logger.isDebugEnabled()) {
					logger.debug("Kerberos mechanism selected from Negotiate header");
				}
				authenticationRequest = new KerberosServiceRequestToken(token.getMechanismToken());
			}

			if (!StringUtils.hasText(token.getMechanism()) || token.getMechanism().equals(SpnegoConstants.NTLMSSP_MECHANISM)) {
				logger.debug("Ntlm mechanism selected from Negotiate header");
				authenticationRequest = new NtlmServiceRequestToken(token.getMechanismToken());
			}

			Authentication authentication;
			try {
				authentication = authenticationManager.authenticate(authenticationRequest);
			} catch (NtlmType2MessageException e) {
				// this happens during the normal course of action of an NTLM
				// authentication
				// a type 2 message is the proper response to a type 1 message
				// from the client
				// see: http://www.innovation.ch/personal/ronald/ntlm.html
				if (logger.isDebugEnabled()) {
					logger.debug("Handling expected NtlmException message: " + e.getMessage());
				}

				String newHeader = null;
				if (token instanceof SpnegoInitToken) {
					SpnegoTargToken authenticationIncompleteToken = new SpnegoTargToken(SpnegoTargToken.ACCEPT_INCOMPLETE,
							SpnegoConstants.NTLMSSP_MECHANISM, e.getToken(), null);

					if (logger.isDebugEnabled()) {
						logger.debug("Sending back accept incomplete header : " + authenticationIncompleteToken);
					}

					newHeader = new String(Base64.encode(authenticationIncompleteToken.toByteArray()), "US-ASCII");
				} else {
					newHeader = new String(Base64.encode(e.getToken()), "US-ASCII");
				}
				

				response.setHeader("WWW-Authenticate", "Negotiate " + newHeader);
	...
	public static void main(String[] args) {
		...
		try {
			String s = "TlRMTVNTUAABAAAAB4IIogAAAAAAAAAAAAAAAAAAAAAFASgKAAAADw==";
			byte[] decode = Base64.decode(s.getBytes());

			System.out.println(SpnegoToken.parse(decode));
